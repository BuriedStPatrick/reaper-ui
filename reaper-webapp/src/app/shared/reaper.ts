...
export interface WwrReply {
  d: number;
  responseText: string;
}

export class ReaperWebRemote implements WebRemote {
  timerFreq: number;
  reqList: string;
  reqList2: string;
  reqRecur: any[];
  ...
  private _reply$ = new BehaviorSubject<WwrReply | null>(null);
  reply$ = this._reply$.asObservable();
  ...
  private runUpdate(): void {
    this.timer = null;
    if (!this.req) {
      this.req = initxmlhttp() as XMLHttpRequest;
    }
    ...
    for (let x = 0; x < this.reqRecur?.length; x++) {
      if (this.reqRecur[x][2] < d) {
        this.reqRecur[x][2] = d + this.reqRecur[x][1];
        this.reqList2 += this.reqRecur[x][0] + ';';
      }
    }

    this.reqList += this.reqList2;
    this.reqList2 = '';

    if (this.reqList !== '') {
      this.req.open('GET', `/_/${this.reqList}`, true);
      this.req.onreadystatechange = () => {
        if (this.req.readyState === 4) {
          ...
          if (this.errcnt > 2) {
            ...
            this.timer = window.setTimeout(this.runUpdate, 100 << (this.errcnt - 3));
          }
          else {
            this.runUpdate();
          }
      ...
      this.timer = window.setTimeout(this.runUpdate, this.timerFreq);
    }
  }
}

export const initxmlhttp = (): XMLHttpRequest | boolean => {
  let xmlhttp: XMLHttpRequest | boolean = false;
  /*@cc_on @*/
  ...
  if (!xmlhttp && (window as any).createRequest) {
    try {
      xmlhttp = (window as any).createRequest();
    } catch (e) {
      xmlhttp = false;
    }
  }
...
// export const runUpdate = (wwr: WebRemote) => {
//   wwr.timer = null;
//   if (!wwr.req) {
//     wwr.req = initxmlhttp() as XMLHttpRequest;
//   }
...
//   // tslint:disable-next-line: prefer-for-of
//   for (let x = 0; x < wwr.reqRecur.length; x++) {
//     if (wwr.reqRecur[x][2] < d) {
//       wwr.reqRecur[x][2] = d + wwr.reqRecur[x][1];
//       wwr.reqList2 += wwr.reqRecur[x][0] + ';';
//     }
//   }

//   wwr.reqList += wwr.reqList2;
//   wwr.reqList2 = '';

//   if (wwr.reqList !== '') {
//     wwr.req.open('GET', `/_/${wwr.reqList}`, true);
//     wwr.req.onreadystatechange = () => {
//       if (wwr.req.readyState === 4) {
//         if (wwr.timer2) {
//           clearTimeout(wwr.timer2);
//           wwr.timer2 = null;
//         }

//         if (wwr.req.responseText !== '') {
//           wwr.errcnt = 0;
//           this.wwr_onreply(wwr.req.responseText, d);
//         } else if (wwr.req.getResponseHeader('Server') == null) {
//           if (wwr.errcnt < 8) {
//             wwr.errcnt++;
//           }
//         }
//         if (wwr.errcnt > 2) {
//           // tslint:disable-next-line: no-bitwise
//           wwr.timer = window.setTimeout(runUpdate, 100 << (wwr.errcnt - 3));
//         }
//         else {
//           this.wwr_run_update();
//         }
...
//     wwr.timer2 = window.setTimeout(() => {
//       wwr.timer2 = null;
//       if (wwr.req.readyState !== 0 && wwr.req.readyState !== 4) {
//         if (wwr.timer) {
//           clearTimeout(wwr.timer);
//           wwr.timer = null;
//         }
...
//     wwr.req.send(null);
//   } else {
//     wwr.timer = window.setTimeout(runUpdate, wwr.timerFreq);
//   }
// };

export const req = (wwr: WebRemote, name: string) =>
  wwr.reqList += `${name};`;

export const reqRecur = (wwr: WebRemote, name: string, interval: number) =>
  wwr.reqRecur.push([name, interval, 0]);

export const reqRecurCancel = (wwr: WebRemote, name: string) => {
  for (let i = 0; i < wwr.reqRecur.length; ++i) {
    if (wwr.reqRecur[i] && wwr.reqRecur[i][0] === name) {
      wwr.reqRecur.splice(i, 1);
      break;
    }
  }
};

export const mkvolstr = (vol: number): string =>
  vol < 0.00000002980232
    ? '-inf dB'
    : `${(Math.log(vol) * 8.68588963806).toFixed(2)} dB`;

export const mkpanstr = (pan: number): string => {
...
export const simpleUnescape = (v): string => String(v)
  .replace(/\\t/g, '\t')
  .replace(/\\n/g, '\n')
  .replace(/\\\\/g, '\\');

export const makeRequest = <T>(url, method): Promise<T | any> => {
